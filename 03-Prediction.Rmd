...
```{r bs-sj-training-testing, fig.cap="(ref:bs-sj-training-testing-caption)"}
# Model (DGP)
known_error <- 2

trainingData <- tibble(
  x = seq(0.8, 2.5, 0.05),
  y = 30 + 15 * (x * sin(x)) + rnorm(n = length(x), mean = 0, sd = known_error),
  truemodel = 30 + 15 * (x * sin(x))
)
...
# Plot training test
mergedData <- rbind(
  data.frame(data = "Training", trainingData),
  data.frame(data = "Testing", testingData)
)

ggplot(mergedData, aes(x = x, y = y)) +
  theme_cowplot(8) +
  geom_point(aes(color = data), alpha = 0.5) +
  geom_line(aes(y = truemodel), color = "black") +
  facet_wrap(~data) +
  xlab("Back Squat relative 1RM") + ylab("Squat Jump Height (cm)") +
  scale_color_manual(values = c(user_blue, user_orange)) +
...
```{r poly-fit-model, fig.height=6, fig.cap="(ref:poly-fit-model-caption)"}
getError <- function(model, newdata) {
  predictions <- predict(model, newdata)
  sqrt(mean((newdata$y - predictions)^2))
}

polymax <- 20

pred.err <- data.frame(poly = seq(1, polymax), Training = 0, Testing = 0)

for (i in seq(1, polymax)) {
  fit <- glm(y ~ poly(x, i), data = trainingData)

  varname <- paste("Polynomial degree", i, sep = " ")
  trainingData[varname] <- predict(fit)

  pred.err$poly[i] <- i
  pred.err$Training[i] <- getError(fit, trainingData)
  pred.err$Testing[i] <- getError(fit, testingData)
}
bestPred <- pred.err$poly[which.min(pred.err$Testing)]

pred.err <- gather(pred.err, key = "set", value = "value", -poly)
...
sampleData$Poly <- factor(sampleData$Poly,
  levels = paste("Polynomial degree", seq(1, polymax), sep = " ")
)

# Plot
ggplot(sampleData, aes(x = x, y = y)) +
  theme_cowplot(8) +
  geom_point(alpha = 0.2, color = user_blue) +
  geom_line(aes(y = truemodel), color = "black") +
  xlab("Back Squat relative 1RM") + ylab("Squat Jump Height (cm)") +
  facet_wrap(~Poly) +
  geom_line(aes(x = x, y = value), color = user_blue)
```
...
```{r testing-training-poly-errors, fig.cap="(ref:testing-training-poly-errors-caption)"}
gg <- ggplot(
  pred.err,
  aes(x = factor(poly), y = value, group = set, color = set)
) +
  theme_cowplot(8) +
  geom_point() +
  geom_hline(yintercept = known_error, color = "gray80") +
  geom_vline(xintercept = bestPred, linetype = "dashed", color = "gray80") +
  geom_line() +
  xlab("Polynomial Degree") +
  ylab("RMSE (cm)") +
  scale_color_manual(values = c(user_orange, user_blue, user_green))

figure_testing_training_perf <- direct.label(gg, "angled.boxes")
figure_testing_training_perf
```
...
```{r cross-validation, fig.cap="(ref:cross-validation-caption)"}
knitr::include_graphics(path = "figures/cross-validation.png")
```
...
```{r}
# Generate sample for predictive demonstration
sample_data <- tibble(
  y = c(15, 19, 28, 28, 30, 57, 71, 88, 95, 97),
  x = y
)
```
...
knitr::kable(
  ...
  booktabs = TRUE,
  digits = 2,
  caption = "(ref:mean-as-prediction-differences-caption)"
)
```
...
We need to *aggregate* these errors or losses into a single metric using the *cost function*. If we calculate the mean of the prediction errors (column `Error` in the Table \@ref(tab:mean-as-prediction-differences)), we are going to get `r round(mean(sample_data$Error), 2)`. This is because the positive and negative errors cancel each other out for the sample `mean` estimator. This error estimator is often referred to as *mean bias error* (`MBE`) or simply *bias* and is often used in *validity* and *reliability* analysis (see [Validity] and [Reliability] sections). 

If we take the mean of the absolute prediction errors (column `Absolute error` in the Table \@ref(tab:mean-as-prediction-differences)) we are going to get *mean absolute error* (`MAE`) estimator, which is in this example equal to `r round(mean(sample_data[[7]]), 2)`. 

If we take the mean of the squared prediction errors (column `Squared error` in the Table \@ref(tab:mean-as-prediction-differences)) we are going to get *mean square error* (`MSE`) estimator, often called *variance* in the case of describing sample dispersion. In this example `MSE` is equal to `r round(mean(sample_data[[6]]), 2)`. To *bring* back `MSE` to the same scale with the observation scale, square root of the `MSE` is taken. This represents *root mean square error* (`RMSE`), which is equal to `r round(sqrt(mean(sample_data[[6]])), 2)`. As explained in [Sample `mean` as the simplest statistical model] section, sample `mean` represents statistical model of the central tendency with the lowest `RMSE`. 
...
```{r loss-function-metrics-example, fig.cap="(ref:loss-function-metrics-example-caption)"}
df <- expand.grid(
  SESOI = 0.25,
  observed = 0,
  predicted = seq(-0.75, 0.75, length.out = 1000)
) %>%
  mutate(
    SESOI_label = paste("SESOI =±", SESOI),
    `Absolute loss` = bmbstats::loss_absolute(observed, predicted, -SESOI, SESOI),
    `Quadratic loss` = bmbstats::loss_quadratic(observed, predicted, -SESOI, SESOI),
    `Huber loss` = bmbstats::loss_huber(observed, predicted, -SESOI, SESOI),
    `Hinge loss` = bmbstats::loss_hinge(observed, predicted, -SESOI, SESOI),
    `Hinge loss (quadratic)` = bmbstats::loss_hinge(observed, predicted, -SESOI, SESOI, exponent = 2)
  ) %>%
  gather(key = "metric", value = "value", -SESOI, -SESOI_label, -observed, -predicted) %>%
  mutate(metric = factor(
    metric,
    levels = c("Absolute loss",
               "Quadratic loss",
               "Huber loss",
               "Hinge loss",
               "Hinge loss (quadratic)")
  ))
...
panel_a <- ggplot(df) +
  ...
  geom_vline(xintercept = 0.25, color = "grey", linetype = "dashed") +
  geom_line(aes(x = predicted, y = value, color = metric)) +
...
```{r mean-cross-validation}
n_folds <- 3
n_repeats <- 5

cv_perf <- bmbstats::cv_model(
  y ~ .,
  sample_data,
  model_func = baseline_model,
  SESOI_lower = -10,
  SESOI_upper = 10,
...
cv_index <- cv_perf$cross_validation$cv_index
cv_index_train <- ifelse(!cv_index, NA, cv_index)
cv_index_test <- ifelse(cv_index, NA, !cv_index)

train_data <- matrix(
  sample_data$x,
  nrow = nrow(cv_index),
  ncol = length(sample_data$x),
  byrow = TRUE
)

test_data <- train_data

train_data <- train_data * cv_index_train
test_data <- test_data * cv_index_test
...
train_data <- apply(
  ...
  function(x) {
    list(
      `Training sample` = paste(ifelse(is.na(x), "--", x), collapse = "  "),
      `mean` = round(mean(x, na.rm = TRUE), 2)
    )
  }
)

train_data <- do.call(rbind.data.frame, train_data)
...
test_data <- apply(
  ...
  function(x) {
    list(
      `Testing sample` = paste(ifelse(is.na(x), "--", x), collapse = "  ")
    )
  }
)

test_data <- do.call(rbind.data.frame, test_data)
test_data$`MBE` <- round(filter(cv_perf$cross_validation$performance$folds$testing, metric == "MBE")$value, 2)
test_data$`MAE` <- round(filter(cv_perf$cross_validation$performance$folds$testing, metric == "MAE")$value, 2)
test_data$`RMSE` <- round(filter(cv_perf$cross_validation$performance$folds$testing, metric == "RMSE")$value, 2)
test_data$`MinErr` <- round(filter(cv_perf$cross_validation$performance$folds$testing, metric == "MinErr")$value, 2)
test_data$`MaxErr` <- round(filter(cv_perf$cross_validation$performance$folds$testing, metric == "MaxErr")$value, 2)

cv_sample_data <- cbind(
  expand_grid(
    Repeat = seq(1, n_repeats),
    Fold = seq(1, n_folds)
  ),
  train_data,
  test_data
...
knitr::kable(
  ...
  booktabs = TRUE,
  row.names = FALSE,
  caption = "(ref:mean-cross-validation-caption)"
) %>%
...
knitr::kable(
  ...
  booktabs = TRUE,
  row.names = FALSE,
  caption = "(ref:repeated-cross-validation-caption)"
) %>%
  kable_styling(font_size = 10)
```
...
cv_metrics <- cv_perf$cross_validation$performance$folds$testing %>%
  filter(metric %in% c("MBE", "MAE", "RMSE", "MinErr", "MaxErr")) %>%
  group_by(metric) %>%
  summarize(value = mean(value)) %>%
  spread(key = "metric", value = "value")

colnames(cv_metrics) <- c("cvMBE", "cvMAE", "cvRMSE", "cvMinErr", "cvMaxErr")
...
knitr::kable(
  ...
  booktabs = TRUE,
  row.names = FALSE,
  caption = "(ref:cv-performance-metrics-caption)"
)
```
...
```{r bias-variance, out.width="100%", fig.cap="(ref:bias-variance-caption)"}
knitr::include_graphics(path = "figures/bias-variance.png")
```
...
- $y_{true}$ value for a particular $x$ values. In our case $SJ$ variable represents target variable $y$, which is equal to $SJ = 30 + 15\times BS\times\sin(BS)$ or $SJ=`r round(trainingData$truemodel[trainingData$x == 1.75], 2)`$cm. Both $x$ and $y_{true}$ values are constant across simulations
...
```{r ten-simulations-bias-variance}
simN <- 200

# Data frame for saving results
modelPerformanceSummary <- data.frame(
  simulation = numeric(0),
  model = numeric(0),
  x = numeric(0),
  y_true = numeric(0),
  y_observed = numeric(0),
  y_predicted = numeric(0)
)

# Simulation -------------------------------------------
for (sim in seq(1, simN)) {
  message(paste("Sim: ", sim))

  trainingDataSim <- tibble(
    x = seq(0.8, 2.5, 0.05),
    y_true = 30 + 15 * (x * sin(x)),
    y_observed = y_true + rnorm(n = length(x), mean = 0, sd = known_error)
  )

  for (i in seq(1, polymax)) {
    fit <- lm(y_observed ~ poly(x, i), data = trainingDataSim)
    modelPerformanceSummary <- rbind(
      modelPerformanceSummary,
      data.frame(
        sim = sim,
        model = i,
        x = trainingDataSim$x[20],
        y_true = trainingDataSim$y_true[20],
        y_observed = trainingDataSim$y_observed[20],
        y_predicted = predict(fit)[20]
      )
    )
  }
}

example_table <- modelPerformanceSummary %>%
  filter(sim <= 10, model == 2)
...
knitr::kable(
  ...
  booktabs = TRUE,
  row.names = FALSE,
  caption = "(ref:ten-simulations-bias-variance-caption)"
)
```
...
```{r example-error-decomposition}
# Analysis --------------------------------------------------------
modelPerformanceSummary <- modelPerformanceSummary %>%
  group_by(model, x) %>%
  summarise(
    y_true = mean(y_true), # Same values, but we need one value
    `Prediction error` = mean((y_predicted - y_true)^2),
    `Bias^2` = (mean(y_predicted) - y_true)^2,
    Variance = mean((mean(y_predicted) - y_predicted)^2),

    # Calculate irreducible error between y_true and y_observed
    `Irreducible error` = mean((y_observed - y_true)^2),

    # This irreducible error needs to be added to MSE
    `Prediction error` = `Prediction error` + `Irreducible error`
  )
...
knitr::kable(
  ...
  booktabs = TRUE,
  row.names = FALSE,
  caption = "(ref:example-error-decomposition-caption)"
)
```
...
```{r bias-variance-simulation, fig.cap="(ref:bias-variance-simulation-caption)"}
modelPerformanceSummary <- gather(
  modelPerformanceSummary,
  key = "metric",
  value = "value", -model, -x, -y_true
)

modelPerformanceSummary$metric <- factor(
  modelPerformanceSummary$metric,
  levels = c("Prediction error", "Bias^2", "Variance", "Irreducible error")
)

# Plot version #1
irreducible_error <- filter(
  modelPerformanceSummary,
  metric == "Irreducible error"
) %>%
  select(model, x, value)

gg <- ggplot(
  filter(modelPerformanceSummary, metric != "Irreducible error"),
) +
  theme_cowplot(8) +
  # geom_point() +
  geom_hline(
    yintercept = mean(irreducible_error$value),
    color = user_grey,
    linetype = "dashed"
  ) +
  geom_line(
    aes(x = factor(model), y = value, group = metric, color = metric),
    alpha = 0.8
  ) +
  xlab("Polynomial Degree") +
  ylab("MSE (cm^2)") +
  scale_color_manual(
    values = c("black", user_blue, user_orange, user_green)
  )

figure_bias_variance_decomposition <- direct.label(gg, "angled.boxes")


# Plot version #2
prediction_error <- filter(
  modelPerformanceSummary,
...
gg <- ggplot(
  filter(modelPerformanceSummary, metric != "Prediction error"),
  aes(x = factor(model), y = value, group = metric, fill = metric)
) +
  theme_cowplot(8) +
  geom_area(alpha = 0.8) +
  geom_line(
    data = prediction_error,
    aes(x = factor(model), y = value, group = 1, fill = NULL),
  ) +
  xlab("Polynomial Degree") +
  ylab("MSE (cm^2)") +
  scale_fill_manual(values = c(user_blue, user_orange, user_green)) +
  theme(
...
# figure_bias_variance_decomposition
gg
```
...
```{r predictive-metrics-mag-based, echo=FALSE}
SESOI_lower <- -1 # cm
SESOI_upper <- 1 # cm

# Feature engineering
trainingData <- trainingData %>%
  select(x, y, truemodel)

predictionData <- cbind(trainingData["y"], poly(trainingData$x, 20, raw = TRUE))

# User-defined performance function
equivalenceSummary <- function(data,
                               lev = NULL,
                               model = NULL,
                               SESOI_lower = 0,
                               SESOI_upper = 0) {
  
  RMSE <- bmbstats::cost_RMSE(observed = data$obs, predicted = data$pred)
  PPER <- bmbstats::cost_PPER(
    observed = data$obs,
    predicted = data$pred,
    SESOI_lower = SESOI_lower,
    SESOI_upper = SESOI_upper)

  c(RMSE = RMSE, PPER = PPER)
}

trControl <- trainControl(
  method = "repeatedcv", repeats = 10, number = 3,
...
caret_model <- train(
  y = predictionData$y, x = select(predictionData, -y),
  method = "glmnet",
  metric = "RMSE",
  trControl = trControl
)
...
# Performance metrics
caret_performance <- getTrainPerf(caret_model)

training_performance <- equivalenceSummary(
  data = data.frame(
    obs = predictionData$y,
    pred = predict(caret_model)
  ),
  SESOI_lower = SESOI_lower,
  SESOI_upper = SESOI_upper
)

table_predictive_metricts <- tibble(
  `SESOI (cm)` = paste("±", SESOI_upper, sep = ""),
  `cvRMSE (cm)` = caret_performance$TrainRMSE,
  `SESOI to cvRMSE` = (SESOI_upper - SESOI_lower) / caret_performance$TrainRMSE,
  `cvPPER` = caret_performance$TrainPPER,
  `RMSE (cm)` = training_performance[1],
  `SESOI to RMSE` = (SESOI_upper - SESOI_lower) / training_performance[1],
  PPER = training_performance[2]
)

knitr::kable(
  table_predictive_metricts,
  digits = 2,
  booktabs = TRUE,
  row.names = FALSE,
  caption = "(ref:predictive-metrics-mag-based-caption)"
)
```

Utilizing *apriori* known SESOI gives us practical *anchor* to evaluate predictive model performance. Reported `SESOI to cvRMSE` (`r round(table_predictive_metricts[3], 2)`) as well as `cvPPER` (`r round(table_predictive_metricts$cvPPER ,2)`) indicate very poor predictive performance of the model. In practical terms, utilizing relative squat 1RM doesn't produce practically meaningful predictions given SESOI of ±1cm and the model as well as the data sample utilized. 
...
```{r elastic-net-prediction, fig.cap="(ref:elastic-net-prediction)"}

trainingData$prediction <- predict(caret_model)
...
panel_a <- ggplot(
  ...
  geom_ribbon(
    aes(
      ymax = prediction + SESOI_upper,
      ymin = prediction + SESOI_lower
    ),
    fill = trivial_color, alpha = 0.2
  ) +
  geom_point(alpha = 0.5, color = user_blue) +
  # geom_line(aes(y = truemodel), color = "black") +
  xlab("Back Squat relative 1RM") +
  ylab("Squat Jump Height (cm)") +
  geom_line(
    aes(x = x, y = prediction),
    color = user_blue
  )

trainingData$residual <- trainingData$y - trainingData$prediction

panel_b <- bmbstats::plot_raincloud_SESOI(
  trainingData,
  value = "residual",
  value_label = "Residual (cm)",
  SESOI_lower = SESOI_lower,
  SESOI_upper = SESOI_upper,
  control = plot_control(
    effect_colors = c(
    user_blue_transparent,
    user_grey_transparent,
    user_blue_transparent
  ),
  cloud_quantile_lines = TRUE
  )) +
  geom_vline(xintercept = 0, color = user_grey_transparent)
...
Let's first estimate predictive performance when predicting MAS scores from the YoYoIR1 score using MAS SESOI ±`r SESOI_MAS_upper`km/h and linear regression.
...
```{r mas-yoyo-ba, fig.cap="(ref:mas-yoyo-ba-caption)"}
mas_yoyo_linear <- bmbstats::cv_model(
  MAS~YoYoIR1,
  yoyo_mas_data,
...
bmbstats::plot_pair_lm(
  group_a = yoyo_mas_data$YoYoIR1,
  group_b = yoyo_mas_data$MAS,
  group_a_label = "YoYoIR1 (m)",
  group_b_label = "MAS (km/h)",
  SESOI_lower = SESOI_MAS_lower,
  SESOI_upper = SESOI_MAS_upper,
  fitted_label = "Predicted MAS (km/h)",
  residuals_label = "Predicted MAS - MAS",
  control = plot_control(smooth_method = "lm")
)
```
...
```{r mas-yoyo-training-performance}
knitr::kable(
  data.frame(t(mas_yoyo_linear$performance)),
  digits = 2,
  booktabs = TRUE,
  row.names = FALSE,
  caption = "(ref:mas-yoyo-training-performance-caption)"
) %>%
  kable_styling(font_size = 10)
```
...
```{r mas-yoyo-cv-performance}
knitr::kable(
  spread(
    mas_yoyo_linear$cross_validation$performance$folds$testing,
    key = "metric",
    value = "value"),
  digits = 2,
  booktabs = TRUE,
  row.names = TRUE,
  caption = "(ref:mas-yoyo-cv-performance-caption)"
) %>%
  kable_styling(font_size = 10)
```
...
```{r mas-yoyo-ba-cross-validated, fig.cap="(ref:mas-yoyo-ba-cross-validated-caption)"}
cowplot::plot_grid(
  plot(mas_yoyo_linear, type = "residuals") + ylim(SESOI_MAS_lower * 1.2, SESOI_MAS_upper * 1.2),
  plot(mas_yoyo_linear, type = "training-residuals") + ylab(NULL) + ylim(SESOI_MAS_lower * 1.2, SESOI_MAS_upper * 1.2),
  plot(mas_yoyo_linear, type = "testing-residuals") + ylab(NULL) + ylim(SESOI_MAS_lower * 1.2, SESOI_MAS_upper * 1.2),
  labels = c("A", "B", "C"),
  label_size = 10,
  align = "hv",
  axis = "l",
...
```{r mas-yoyo-bias-variance-index, fig.cap="(ref:mas-yoyo-bias-variance-index-caption)"}
plot(mas_yoyo_linear, type = "bias-variance-index")
```
...
```{r mas-yoyo-bias-variance-y-obs, fig.cap="(ref:mas-yoyo-bias-variance-y-obs-caption)"}
plot(mas_yoyo_linear, type = "bias-variance-observed")
```
...
```{r mas-yoyo-bias-cv-prediction-index, fig.cap="(ref:mas-yoyo-bias-cv-prediction-index-caption)"}
plot(mas_yoyo_linear, type = "prediction-index")
```
...
```{r mas-yoyo-bias-cv-prediction-observation, fig.cap="(ref:mas-yoyo-bias-cv-prediction-observation-caption)"}
plot(mas_yoyo_linear, type = "prediction-observed")
```
...
```{r mas-yoyo-cv-performance-summary}
knitr::kable(
  mas_yoyo_linear$cross_validation$performance$summary$overall,
  digits = 2,
  booktabs = TRUE,
  row.names = FALSE,
  caption = "(ref:mas-yoyo-cv-performance-summary-caption)"
)
```
...
```{r mas-yoyo-cv-graphical, fig.cap="(ref:mas-yoyo-cv-graphical-caption)"}
plot(mas_yoyo_linear, type = "estimators")
```
...
Overall, predicting MAS from the YoYoIR1 score, *given* the data collected, SESOI of ±0.5km/h, and linear regression as a model, is practically excellent. Please note that prediction can be practically useful (given SESOI) (`PPER`; `r paste("CV from", round(filter(mas_yoyo_linear$cross_validation$performance$summary$testing, metric == "PPER")["min"], 2), "to", round(filter(mas_yoyo_linear$cross_validation$performance$summary$testing, metric == "PPER")["max"], 2))`) even when `R-squared` is relatively low (`r paste("CV from", round(filter(mas_yoyo_linear$cross_validation$performance$summary$testing, metric == "R-squared")["min"], 2), "to", round(filter(mas_yoyo_linear$cross_validation$performance$summary$testing, metric == "R-squared")["max"], 2))`). And the *vice versa* in some cases. That's the reason why we need to utilize magnitude-based estimators as a complement of contemporary estimators such as `R-squared`, `RSE`, and `RMSE`. 
...
As shown in [Describing relationship between two variables], predicting YoYoIR1 from MAS scores was not practically useful (or precise enough). But for the sake of completeness, let's perform cross-validated prediction (using 3 folds and 5 repeats). YoYoIR1 SESOI is taken to be ±`r SESOI_yoyo_upper`m. 
...
```{r yoyo-mas-ba, fig.cap="(ref:yoyo-mas-ba-caption)"}
yoyo_mas_linear <- bmbstats::cv_model(
  YoYoIR1~MAS,
  yoyo_mas_data,
...
bmbstats::plot_pair_lm(
  group_b = yoyo_mas_data$YoYoIR1,
  group_a = yoyo_mas_data$MAS,
  group_b_label = "YoYoIR1 (m)",
  group_a_label = "MAS (km/h)",
  SESOI_lower = SESOI_yoyo_lower,
  SESOI_upper = SESOI_yoyo_upper,
  fitted_label = "Predicted YoYoIR1 (m)",
  residuals_label = "Predicted YoYoIR1 - YoYoIR1",
  control = plot_control(smooth_method = "lm")
)
```
...
```{r yoyo-mas-cv-performance-summary}
knitr::kable(
  yoyo_mas_linear$cross_validation$performance$summary$overall,
  digits = 2,
  booktabs = TRUE,
  row.names = FALSE,
  caption = "(ref:yoyo-mas-cv-performance-summary-caption)"
)
```
