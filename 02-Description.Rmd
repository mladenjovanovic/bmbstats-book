...
table_effect_size_stats_independent_groups <- height_data_wide %>%
  summarize(
    ...
    `Cohen's d` = bmbstats::cohens_d(Female, Male, paired = FALSE),
...
```{r perc-change-vs-cohensd}
change_vs_cohen <- tribble(
...
knitr::kable(
  ...
  booktabs = TRUE,
  digits = 2,
  caption = "(ref:perc-change-vs-cohensd-caption)"
)
```
...
```{r magnitudes-of-effect}
magnitude_of_effect <- tribble(
...
knitr::kable(
  ...
  booktabs = TRUE,
  digits = 2,
  caption = "(ref:magnitudes-of-effect-caption)"
) %>% 
  kable_styling(latex_options = c("scale_down"))
```
...
```{r magnitudes-of-effect-CLES-OVL}
magnitude_of_effect <- tribble(
  ~`Magnitude of effect`, ~`Trivial`, ~`Small`, ~`Moderate`, ~`Large`, ~`Very Large`, ~`Nearly Perfect`,
  "Cohen's d", "0 - 0.2", "0.2 - 0.6", "0.6 - 1.2", "1.2 - 2.0", "2.0 - 4.0", "> 4.0",
...
knitr::kable(
  ...
  booktabs = TRUE,
  digits = 2,
  caption = "(ref:magnitudes-of-effect-CLES-OVL-caption)"
) %>% 
  kable_styling(latex_options = c("scale_down"))
```
...
bmbstats::plot_pair_lm(
  predictor = relationship$`YoYoIR1 (m)`,
  outcome = relationship$`MAS (km/h)`,
  SESOI_lower = SESOI_MAS_lower,
  SESOI_upper = SESOI_MAS_upper,
  predictor_label = "YoYoIR1 (m)",
  outcome_label = "MAS (km/h)",
...
bmbstats::plot_pair_lm(
  outcome = relationship$`YoYoIR1 (m)`,
  predictor = relationship$`MAS (km/h)`,
  SESOI_lower = SESOI_yoyo_lower,
  SESOI_upper = SESOI_yoyo_upper,
  outcome_label = "YoYoIR1 (m)",
  predictor_label = "MAS (km/h)",
  control = plot_control(points_fill = female_color, points_color = female_color)
)
```
